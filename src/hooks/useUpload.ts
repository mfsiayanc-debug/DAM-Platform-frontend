...
export function useUpload(onComplete?: (assetIds: string[]) => void) {
    ...
    try {
      ...
      newJobs.forEach(job => {
        setUploadJobs(prev => prev.map(j => 
          j.id === job.id 
            ? { ...j, status: 'processing' as const, progress: 100 }
            : j
        ));
      });
      ...
      newJobs.forEach(job => {
        setUploadJobs(prev => prev.map(j => 
          j.id === job.id 
            ? { ...j, status: 'failed' as const, error: errorMessage }
            : j
        ));
      });
  ...
  const pollForAssetProcessing = useCallback(async (assetIds: string[], jobs: UploadJob[]) => {
    let attempts = 0;
      ...
      try {
        ...
        if (allProcessed || attempts >= config.polling.maxAttempts) {
          ...
          if (allProcessed) {
            ...
            toast.warning('Processing is taking longer than expected. Please refresh to see the latest status.');
          }
        }
      } catch (err) {
